...
def fetch_ohlcv(symbol: str, timeframe: str, count: int) -> pd.DataFrame:
...
def breakout_and_retest(
    ...
    broke_high = bool(current["close"] > reference["high"].max())
    broke_low = bool(current["close"] < reference["low"].min())
    ...
    for index in range(max(1, len(df) - 6), len(df) - 1):
        ...
        high_level = float(prior["high"].max())
        low_level = float(prior["low"].min())

        if breakout["close"] > high_level:
            near = abs(float(current["close"]) - high_level) / high_level <= tolerance
            touched = current["low"] <= high_level * (1 + tolerance)
            bullish_retest |= bool(near and touched and current["close"] >= high_level)

        if breakout["close"] < low_level:
            near = abs(float(current["close"]) - low_level) / low_level <= tolerance
            touched = current["high"] >= low_level * (1 - tolerance)
            bearish_retest |= bool(near and touched and current["close"] <= low_level)
...
def build_context(df: pd.DataFrame) -> MarketContext:
    ...
    broke_high, broke_low, bullish_retest, bearish_retest = breakout_and_retest(df)
...
def moving_average_signal(context: MarketContext) -> Signal:
    ...
    bullish = ema["ema_9"] > ema["ema_21"] and ema9_slope > 0 and ema21_slope > 0
    bearish = ema["ema_9"] < ema["ema_21"] and ema9_slope < 0 and ema21_slope < 0
...
def vwap_signal(context: MarketContext) -> Signal:
    price = float(context.df["close"].iloc[-1])
    above = price > context.vwap
    rising = context.vwap_slope_pct > 0
    falling = context.vwap_slope_pct < 0
    reasons: list[str] = []
...
def confluence_signal(
    ...
    multiplier = {0: 0.60, 1: 0.60, 2: 0.85, 3: 1.0, 4: 1.10}[agreeing]
