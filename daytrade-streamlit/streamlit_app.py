...
def run_scanner(symbols: list[str], timeframe: str, count: int, risk_budget: float | None) -> pd.DataFrame:
    rows = []
    ...
    for i, symbol in enumerate(symbols):
        ...
        if error:
            rows.append({"Ativo": symbol, "Direção": "ERRO", "Score": None, "Setup": error[:60],
                        ...
                        "Entrada": None, "Stop": None, "Alvo 1": None, "Quantidade": None,
                        "Total (R$)": None, "Confiança": None})
            continue
...
if mode == "Análise individual":
    ...
    if run_scanner_clicked:
        st.session_state.scanner_result = run_scanner(st.session_state.watchlist, timeframe, count, risk_budget)
