...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        # Sincroniza com o remoto antes de enviar -- evita falha de push
        # quando alguem editou outro arquivo direto pelo navegador do
        # GitHub entre uma execucao e outra deste script.
        git pull origin main --no-edit | Out-Null

        git push
        if ($LASTEXITCODE -ne 0) {
            Log "ERRO: git push falhou mesmo apos pull. Verifique o token/permissoes."
            exit 1
        }
