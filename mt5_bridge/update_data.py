...
from daytrade_smc import _fetch_ohlcv_mt5, load_symbols  # noqa: E402
 
OUTPUT_PATH = Path(__file__).resolve().parent.parent / "data" / "mt5_snapshot.json"
TIMEFRAMES_TO_FETCH = ["M15", "H1", "H4", "D1", "W1"]
CANDLES_PER_TIMEFRAME = {"M15": 300, "H1": 300, "H4": 200, "D1": 300, "W1": 200}
 
 
def main() -> None:
    symbols = load_symbols()
    print(f"Atualizando {len(symbols)} ativo(s) via MT5: {', '.join(symbols)}\n")
    ...
    for symbol in symbols:
        ...
        for tf in TIMEFRAMES_TO_FETCH:
            try:
                df = _fetch_ohlcv_mt5(symbol, tf, CANDLES_PER_TIMEFRAME[tf])
                records = [
                    {
                        "time": idx.isoformat(),
                        "open": float(row["open"]),
                ...
                ok_count += 1
            except Exception as exc:  # noqa: BLE001 — precisa continuar pros outros ativos/timeframes mesmo se um falhar
